  ...
  return useQuery<ProofSetsResponse, Error>({
    ...
    queryFn: async () => {
      ...
      const providerUrlMap = new Map(
        providers.map((provider: Provider | any) => [provider.owner, provider.pdpUrl])
      );
      ...
      const proofsetDetailsPromises = proofsets.map(async (proofset) => {
        ...
        try {
          ...
          const provider = providers.find(
            (p: Provider | any) => p.owner === proofset.payee
          );
          ...
          return {
            ...
            pdpUrl,
            provider:
              providers.find((p: Provider | any) => p.owner === proofset.payee) ||
              null,
          };
        }
      });
